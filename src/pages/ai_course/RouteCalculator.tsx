...
export type Point = {
  ...
  export const RouteCalculator = (points: Point[]) => {
    ...
    // 출발지와 도착지를 제외한 나머지 지점들에 대해 방문 여부를 false로 설정
    ...
    // 출발지와 도착지는 방문했다고 표시
    visited[0] = true;
    visited[points.length - 1] = true;
  
    const result = shortestRoute(points, points[0], visited, 0, [points[0]]);
  
    // 도착지를 경로에 추가
    result.path.push(points[points.length - 1]);
    result.length += calculateDistance(result.path[result.path.length - 2], points[points.length - 1]);
  
    console.log("The shortest route length is: ", result.length);
    console.log("The shortest route is: ", result.path);
